---
title: "M1 Wk2: Instructions"
description: "Familiarizing Yourself with Obsidian"
date: 2025-07-17
draft: false
weight: -12
Authors: Coleman 2025
---

## Obsidian.md and Practical applications for historical research.
### Week Two (Sept. 15-19, 2025)

This week, consider a historical or archaeological topic you are interested in and begin exploring this topic with [Obsidian.md](https://obsidian.md/). 

{{< notice success "Important" >}} By all means, try some of the more technical aspects of Obsidian. But remember: with tech work, if it doesn't come together in about 30 minutes, it won't come in an hour. So take a break. Close the laptop. Call somebody up for help. Find another pair of eyes to look at the problem. I don't want to hear that you labored heroically for 2 hours to do something. Jump into our social space and ask for advice.
{{< /notice >}}

### **Overview**
1) **Plugin of the Week:** **Relay & Hypothes.is**

2) **Rationale:** To familiarize yourself with the core functionality of Obsidian and Markdown language. Installing Mixa allows you to see from day one how your private vault can be transformed into a public-facing website, framing the purpose of the course. DO NOT STRESS if it does not work. I have a tutorial for you to follow. If that doesn't work, please contact me and I'll walk you through it. 

3) **Asynchronous Activities:** Read and annotate assigned readings and video using Hypothes.is and Obsidian.md.

4) **Assignment:**  

### **Log your work**

{{< notice success "Important" >}}
For your digital work, it is critical that you keep notes on what works, what doesn't, what error messages you received, what help you received from others, what websites you went to, and so on.
{{< /notice >}}

---
**Mandatory Reading/Listening:**
1. Roddick, Andy. 2022 _From Info-Glut to Connected Notes: Obsidian and Digital Note-Taking in Academia_ DMDS Workshops. [Obsidian and Digital-Note Taking in Academia](https://scds.github.io/dmds-22-23/Obsidian.html)
2. [Getting Started with Markdown | Programming Historian](https://programminghistorian.org/en/lessons/getting-started-with-markdown)
3. [Sustainable Authorship in Plain Text using Pandoc and Markdown | Programming Historian](https://programminghistorian.org/en/lessons/sustainable-authorship-in-plain-text-using-pandoc-and-markdown)

**Additional Resources**

Ahrens, Sönke. 2017   _How to Take Smart Notes : One Simple Technique to Boost Writing, Learning and Thinking - for Students, Academics and Nonfiction Book Writers._ Place of publication is not identified_. Sönke Ahrens. 

**Sönke Ahrens'  book, _How to Take Smart Notes_, is meant for you to skim through for the duration of the course. This is not, strictly speaking, a peer-reviewed academic book, but it is a great source to learn how to create notes or Atomic notes. As Andy Roddick (2022) observes, making notes, not taking notes, forces you to think more critically about the ideas you formulate and record as notes.**

---



